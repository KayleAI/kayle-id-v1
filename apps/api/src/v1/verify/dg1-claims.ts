...
// Face-match threshold window, calibrated for the AuraFace recognizer
// (ArcFace-family normalized cosine). The window slides between MIN
// and MAX based on document age + youth drift — older docs and
// faster-changing faces get a more permissive threshold. Mapping
// back to raw cosine via `(cos + 1) / 2`: MAX=0.80 → raw 0.60
// (strict, fresh adult docs), MIN=0.6875 → raw 0.375 (the floor for
// IDV — comfortably above the InsightFace published same-person
// threshold of raw 0.28-0.30 but permissive enough to absorb
// real-world ageing on aged child docs). The prior 0.75-0.90 window
// was calibrated for SFace and false-rejected legitimate same-person
// pairs even after the age-gap discount.
