...
describe("session analytics helpers", () => {
  ...
  test("runs expired-session normalization on 5-minute boundaries", () => {
    expect(
      shouldRunExpiredSessionNormalization(new Date("2026-03-21T12:00:00.000Z"))
    ).toBeTrue();
    expect(
      shouldRunExpiredSessionNormalization(new Date("2026-03-21T12:05:00.000Z"))
    ).toBeTrue();
    expect(
      shouldRunExpiredSessionNormalization(new Date("2026-03-21T12:06:00.000Z"))
    ).toBeFalse();
  });
