	...
	return (
		<main className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
			...
			<div className="grid gap-6 md:grid-cols-2 lg:gap-8">
				{options.map((option) => (
					<Link
						className="group rounded-2xl border border-neutral-100 bg-neutral-50/60 p-8 transition-colors hover:border-neutral-200 hover:bg-neutral-50"
						key={option.to}
						to={option.to}
					>
						<option.Icon className="text-emerald-700" size={56} />
						<h2 className="mt-6 font-light text-2xl text-neutral-950 tracking-tight">
							{option.title}
						</h2>
						<p className="mt-3 text-neutral-600 text-pretty leading-relaxed">
							{option.description}
						</p>
						<span className="mt-6 inline-flex items-center font-medium text-emerald-700 text-sm group-hover:underline">
							{option.cta}
			...
			<section className="mt-6 grid gap-3 md:grid-cols-2 md:gap-4">
				{legalLinks.map((link) => (
					<Link
						className="group rounded-2xl border border-neutral-100 bg-neutral-50/60 px-5 py-4 transition-colors hover:border-neutral-200 hover:bg-neutral-50"
						key={link.to}
						to={link.to}
					>
						<div className="flex items-center justify-between gap-3">
							<h3 className="font-medium text-neutral-950 text-sm">
								{link.title}
							</h3>
							<ArrowUpRightIcon
								aria-hidden="true"
								className="size-4 shrink-0 text-neutral-400 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-neutral-700"
							/>
						</div>
