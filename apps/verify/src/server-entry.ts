...
// TanStack Start's exported server.fetch types itself as `(request) => ...`,
// but the Cloudflare Workers runtime calls it with the standard
// (request, env, ctx) triple and TanStack forwards them through. Cast so we
// can pass env/ctx without dropping them or threading them via globals.
