...
After the verify pipeline accepts an attempt: SOD chain verified, all data-group hashes match, active and chip authentication checks pass when applicable, the face matcher accepts the selfie, and the user has consented to share at least all required fields.
