...
sequenceDiagram
    ...
    API-->>Mobile: ServerVerdict (accepted | rejected)
...
Three independent checks run before a verification is accepted:
...
2. **Active authentication** — If the document carries a DG15, the chip signs a freshly derived challenge with its private key. A successful signature proves the chip you're talking to is the same chip that was personalised at issuance, not a clone holding the same data.
...
The selfie is matched against the on-chip portrait by the biometric verifier service. The decision is fail-closed: if the verifier is unavailable, the verdict is rejection. In production a fallback path is treated as a rejection even if the score is high.
...
Mobile clients drive the verification over a single WebSocket. The wire format is [Cap'n Proto](https://capnproto.org), defined in `packages/capnp/verify.capnp`. The protocol has a small message vocabulary: a hello, a handful of phase updates, chunked binary data payloads (DG1, DG2, DG14, DG15, SOD, selfie frames, active-auth signatures), and a server verdict.
