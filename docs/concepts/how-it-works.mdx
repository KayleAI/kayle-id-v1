...
sequenceDiagram
    ...
    participant FaceMatch as Face matcher
    ...
    API->>FaceMatch: DG2 portrait + selfies
    FaceMatch-->>API: { passed, score }
...
The selfie is matched against the on-chip portrait by the face matcher service. The decision is fail-closed: if the matcher is unavailable, the verdict is rejection. In production a fallback path is treated as a rejection even if the score is high.
