...
<Steps>
  ...
  <Step title="Create a verification session">
    A session represents one identity verification you want to perform. You can request specific [share fields](/verifications/share-fields) — claims the user must consent to share before completing the flow.
  ...
  <Step title="Send the user through the flow">
    Redirect the user to `verification_url`. They'll land on `verify.kayle.id`, see the consent screen for the share fields you requested, and complete the flow on their phone (NFC chip read of their passport plus a selfie).
